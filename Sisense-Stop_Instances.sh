...
usage() {
    ...
    echo "Options:"
    echo "  -r <region>  Stop instances in specific regions"
    echo "  -i <instance_id>          Stop a specific instance by ID"
...
if [ -z "$instance_id" ]; then
    echo "instance_id is obligatory, please provide one" 
    exit 1
fi
...
aws_command="aws ec2 stop-instances"
...
if [ -z "$result" ]; then
    ...
    echo "Instances and their states:"
    echo "$result"
fi
